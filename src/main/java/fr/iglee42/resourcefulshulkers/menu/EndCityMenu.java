...
public class EndCityMenu extends AbstractContainerMenu {

    public static final int[][] SHULKER_SLOTS = new int[][] {
            {26, 18}, {8, 36}, {44, 36}, {26, 54},
            {8, 18},{44, 18}, {8, 54}, {44, 54}
    };


    private final EndCityBlockEntity blockEntity;
    private final Level level;

    public EndCityMenu(int id, Inventory inv, FriendlyByteBuf extraData){
        this(id,inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public EndCityMenu(int id, Inventory playerInv, BlockEntity entity) {
        super(RSBlockEntities.END_CITY_MENU.get(),id);
        this.blockEntity = (EndCityBlockEntity) entity;
        ...
        IItemHandler shells = blockEntity.getShells();
        for(int l = 0; l < shells.getSlots(); ++l) {
            this.addSlot(new SlotItemHandler(shells, l, 8 + l * 18, 86).setBackground(InventoryMenu.BLOCK_ATLAS,EMPTY_SHELL_SLOT));
        }
    ...
    public @NotNull ItemStack quickMoveStack(Player playerIn, int index) {
        int TE_INSERTABLE_INVENTORY_SLOT_COUNT = blockEntity.getUpgrades().getSlots() + blockEntity.getShells().getSlots() + blockEntity.getShulkers().getSlots();
    ...
    public EndCityBlockEntity getBlockEntity() {
        return blockEntity;
    }
}
