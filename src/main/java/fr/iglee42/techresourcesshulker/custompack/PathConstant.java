...
public class PathConstant {
    ...
    public static void init() {
        try {
            deleteDirectory(FMLPaths.CONFIGDIR.get().resolve("techresourcesshulker/pack"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        ROOT_PATH = FMLPaths.CONFIGDIR.get().resolve("techresourcesshulker/pack");
        ROOT_PATH.toFile().mkdirs();
        ASSETS_PATH = ROOT_PATH.resolve("assets/techresourcesshulker");
        DATAS_PATH = ROOT_PATH.resolve("data/techresourcesshulker");
