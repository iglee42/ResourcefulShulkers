...
public class ShulkersManager {
    ...
    public static void init(){
        //TYPES.add(new Resource(new ResourceLocation(TechResourcesShulker.MODID,"wood"),Items.OAK_LOG,DyeColor.BROWN,0X612B02,null));

        File dir = FMLPaths.CONFIGDIR.get().resolve(TechResourcesShulker.MODID + "/shulkers/").toFile();
        ...
        if (dir.isDirectory()) {
            ...
            for (File file : files) {
                ...
                try {
                    ...
                    TechResourcesShulker.LOGGER.error("An error occurred while loading shulkers (" + file.getName()+")", e);
                } finally {
        ...
        if (dir.mkdirs() && dir.isDirectory()){
            for (BaseTypes t : BaseTypes.values()) {
                try (FileWriter writer = new FileWriter(new File(dir,t.name().toLowerCase() + ".json"))) {
                    ...
                    TechResourcesShulker.LOGGER.error("An error occurred while generating default shulkers", e);
                }
